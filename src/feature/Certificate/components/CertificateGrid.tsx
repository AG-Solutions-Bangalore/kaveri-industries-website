        ...
        <div className="mt-10 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
          {MAIN_CERTIFICATES.map((cert) => (
            ...
            >
              ...
              <div className="relative aspect-3/4 w-full overflow-hidden bg-slate-100 p-3 dark:bg-slate-950">
                <img
                  src={cert.image}
                  alt={cert.imageAlt}
              ...
              <div className="flex flex-1 flex-col p-5">
                ...
                {/* Metadata Table */}
                <div className="mt-4 space-y-1.5 border-t border-slate-100 pt-3 text-[11px] text-slate-600 dark:border-slate-800 dark:text-slate-400 sm:text-xs">
                  <div className="flex justify-between">
                    <span className="text-slate-400 dark:text-slate-500">Certificate No.</span>
                    <span className="font-semibold text-slate-700 dark:text-slate-200">{cert.certNo}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400 dark:text-slate-500">Issued By</span>
                    <span className="font-semibold text-slate-700 dark:text-slate-200">{cert.issuedBy}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-400 dark:text-slate-500">Valid Until</span>
                    <span className="font-semibold text-slate-700 dark:text-slate-200">{cert.validUntil}</span>
                  </div>
                </div>
