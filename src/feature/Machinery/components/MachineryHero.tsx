...
export function MachineryHero() {
  const getIcon = (icon: "cpu" | "factory" | "crosshair" | "users") => {
    switch (icon) {
      case "cpu":
        return <Cpu className="h-4 w-4 text-brand-400" />;
      case "factory":
        return <Factory className="h-4 w-4 text-brand-400" />;
      case "crosshair":
        return <Crosshair className="h-4 w-4 text-brand-400" />;
      case "users":
  ...
  return (
    <div className="relative text-white overflow-hidden bg-slate-900">
      ...
      <div className="pointer-events-none absolute inset-0 overflow-hidden bg-slate-900">
        ...
        {/* Gradient overlays for high text contrast and industrial sparks visibility */}
        <div className="absolute inset-0 bg-gradient-to-r from-[#071120] from-25% via-[#071120]/85 via-60% to-transparent" />
        <div className="absolute inset-0 bg-gradient-to-b from-[#071120]/60 via-transparent to-[#071120]/85" />
      </div>

      <section
        aria-labelledby="machinery-hero-heading"
      ...
      >
        ...
        <div className="grid items-center gap-10 lg:grid-cols-12 lg:gap-8">
          ...
          <div className="lg:col-span-8">
            <motion.h1
              id="machinery-hero-heading"
              initial={{ opacity: 0, y: 16 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.5, ease: EASE }}
              className="text-3xl font-extrabold tracking-tight sm:text-4xl lg:text-5xl"
            >
              Machinery & <br className="hidden sm:inline" />
              <span className="text-brand-400">Manufacturing Facilities</span>
            </motion.h1>
            ...
            >
              Equipped with advanced and well-maintained machinery to deliver
              high-quality products with precision, consistency and efficiency.
            </motion.p>
            ...
            >
              {MACHINERY_HERO_PILLS.map((pill, idx) => (
                <div
                  key={pill.line1}
                  className={`flex items-center gap-3 ${
                    idx < MACHINERY_HERO_PILLS.length - 1
